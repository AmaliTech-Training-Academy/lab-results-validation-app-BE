...
-- of a DB-backed default row here, applying to every eligible cohort (cohort_id IS NULL) exactly
-- as the old fixed job did. SyncScheduleService picks this up on startup (rehydrate()) and via the
-- normal /api/v1/sync-schedules CRUD endpoints, so the frontend can read and change it instead of
-- it being baked into app config.
