...
/**
 * Read-only access to SharePoint drives via Microsoft Graph — the production implementation of
 * {@link GraphDriveService}.
 *
 * <p>Active unless {@code validata.sharepoint.source} says otherwise, so existing deployments and
 * every current configuration keep this behaviour with no change. Set that property to
 * {@code fixtures} to swap in {@link FixtureDriveService} instead.
 *
...
public class MicrosoftGraphDriveService implements GraphDriveService {
    ...
    public MicrosoftGraphDriveService(
